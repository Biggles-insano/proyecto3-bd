...
CREATE TABLE Factura_Maestro ( id_factura_maestro INT PRIMARY KEY AUTO_INCREMENT, numero_factura VARCHAR(50) NOT NULL, cliente_id INT, fecha_factura DATE NOT NULL, nit_cliente VARCHAR(20) NOT NULL, total DECIMAL(10, 2) NOT NULL, -- Monto total de la factura total_pagado DECIMAL(10, 2) DEFAULT 0.00, -- Monto pagado hasta el momento estado_factura VARCHAR(20) NOT

NULL, -- Estado de la factura (pendiente, pagada, anulada) metodo_pago VARCHAR(50), -- Forma de pago (efectivo, tarjeta, etc.) FOREIGN KEY (cliente_id) REFERENCES Cliente(id_cliente) );
